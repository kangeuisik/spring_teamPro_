...
// ========================== 검색관련 ========================================	
	...
	$('.subCate option').each(function(index, item){
		if($(item).attr('class')==cate_id){
			$(item).show();
			$('.subCate option:eq(0)').show(); // 맨 첫번째 옵션은 무조건 나오게
		} else {
			$(item).hide();
		}
	})
	...
	$('.searchBookForm button').on('click',function(){
		...
		searchForm.append($('<input/>', {type: 'number', name: 'perPageNum', value: ''+parseInt(perPage.val()) }))
			.appendTo('body')
			.submit();
	});
...
// ======================================상세페이지에서 쓰는 거 ==============================================	
	...
	let count = amountObj.val(); // 디폴트값=1
	// 수량감소
