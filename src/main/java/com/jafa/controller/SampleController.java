...
public class SampleController {
	...
	public Map<String, Object> detail(int bookNo) {
		...
		map.put("cateList", cateList); // 카테고리목록
		map.put("subCateList", subCateList); // 서브카테고리 목록
		return map;
	}
	...
	public Map<String, Object> addCart(@RequestBody CartVO vo) {
		...
		boolean check = false; // 중복여부 기본 false
		String message ="";
		for (CartVO list : cartList) {
			if(vo.getBookNo() == list.getBookVO().getBookNo()) { // 장바구니 목록에 같은 책 있으면...
				check = true; // 중복여부 true
				message+="이미 장바구니에 있다!";
			}
		}
		if(!check) { // 중복여부 true가 아니면
